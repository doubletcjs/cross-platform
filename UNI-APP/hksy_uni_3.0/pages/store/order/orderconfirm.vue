<template>
  <view class="order-confirm-body" v-if="product != null">
    <view class="order-confirm-title-wapper">
      <view class="order-confirm-title">确认订单</view>
    ...
    <view class="order-confirm-item-wapper" style="margin-top: 24rpx;">
      <view class="order-confirm-item-name">商品</view>
      <view class="order-confirm-item-name" style>{{product.productName}}</view>
    </view>
    <view class="order-confirm-item-wapper">
      <view class="order-confirm-item-name">售卖价格</view>
      <view
        class="order-confirm-item-name"
        style="text-align: right;"
      >{{parseFloat(price).toFixed(2)}} 个</view>
    </view>
    <view class="order-confirm-item-wapper">
      <view class="order-confirm-item-name">当前余额</view>
      <view
        class="order-confirm-item-name"
        style="margin-left: 40rpx; max-width: 410rpx;"
      >{{parseFloat(coin).toFixed(2)}} 个</view>
    </view>
    <view class="order-confirm-tip" v-if="price - coin > 0">余额不足</view>
    <view class="order-confirm-function-wapper" :style="buttonStyle">
      <view
        class="order-confirm-button-text"
        style="background-color: rgba(204, 204, 204, 1);border-radius: 15rpx;"
        @tap="colseAction"
      >取 消</view>
      <view
        v-if="price - coin > 0"
        class="order-confirm-button-text"
        style="background:rgba(23,96,255,1);border-radius: 15rpx;margin-left: 44rpx;"
        @tap="buyFunctionAction"
      >立即充值</view>
      <view
        v-else
        class="order-confirm-button-text"
