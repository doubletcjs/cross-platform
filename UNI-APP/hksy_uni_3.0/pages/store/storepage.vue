<template>
  <view class="product-list-wapper">
    ...
    >
      <view
        class="product-cell-image-wapper"
        style="background-image: url(./static/images/main_product_thumbnail@3x.png);"
      >
        <view class="product-cell-image-content-wapper">
          <view class="product-cell-image-content-title">HK iCloud-{{product.productType}}T</view>
          <view class="product-cell-image-content-tip">- 为IPFS量身定制</view>
        </view>
      </view>
      <view class="product-cell-name">{{product.productName}}</view>
      <view class="product-cell-price">{{parseFloat(product.productPreferentialPrice).toFixed(2)}}/个</view>
    </view>
    ...
    <view style="width: 100%; background-color: rgba(21, 25, 54, 1);" v-if="productList.length > 0">
      <uni-load-more
        iconType="auto"
        :status="moreStatus"
        :contentText="this.util.moreContentText"
        @clickLoadMore="loadMoreProduct"
      ></uni-load-more>
    </view>
...
export default {
  ...
  methods: {
    ...
    productSelect(res) {
      ...
      var productId = this.productList[productIndex].guid;
