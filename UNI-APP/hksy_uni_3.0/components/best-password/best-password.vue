<template>
	<view class="best-modal digital-keyboard-modal" :class="{'best-modal-active':_show}">
		...
		<view class="best-modal-content">
			...
			<view class="dk-subtitle" v-if="!Details">请输入支付密码</view>
			<view class="dk-subtitle" style="font-size: 32rpx;" v-else>{{ Details }}</view>
			...
			<view class="pwd-forget">
				<text v-if="_forget" @tap="forgetPwd">忘记密码</text>
			</view>
			<view class="digital-keyboard">
				<view class="form_edit clearfix">
					<view class="num" hover-class="numActive" v-for="item in digitalList" :key="item" :class="{'no-num':(item === '' || item === '-1')}"
...
<style scoped>
	...
	.numActive{
		background-color: rgba(130,221,204,1) !important;
