<template>
	<view style="background-color: rgba(247, 246, 245, 1);">
		<view class="store-ower-content">
			<!-- 店铺 -->
			<view class="store-base-row" style="width: 78%;" @click="storeDetail">
				<!-- 头像 -->
		...
		<view class="store-tab">
			...
			<!-- 分类 -->
			<view class="store-base-row" style="position: absolute; right: 32rpx;" @click="storeClassify">
...
<script>
	export default {
		...
		methods: {
			...
			storeDetail() {
				uni.navigateTo({
					url: "./store/home_page_store_detail"
				})
			},
			//商品分类
			storeClassify() {
				uni.navigateTo({
					url: "./store/home_page_store_classify"
				})
			},
		},
		created() {
