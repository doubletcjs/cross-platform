...
Page({
  ...
  attentionAnchor() {
    ...
    if (this.data.attention) {
      this.setData({
        alertvisible: true,
        contentString: '关注成功，系统会在开播前10分钟推送相关信息，请留意。'
      })
    } else {
      this.setData({
        alertvisible: true,
        contentString: '已取消关注，将不会收到活动相关信息，如有需要，请重新关注，谢谢！'
      })
    }
  ...
  give() {
    ...
    if (this.data.giveOff) {
      this.setData({
        grveNum: this.data.grveNum + 1,
        grveNumAnimation: true,
        giveOff: false
      })
      setTimeout(() => {
        this.setData({
          grveNumAnimation: false,
          giveOff: true
        })
      }, 1600)
    }
